...
all_services_ok=1
for service in "${ENV_SERVICES[@]}"; do
  env_config::validate_service "${COMPLETE_DIR}" "${service}" >/dev/null 2>&1 || all_services_ok=0
done
check "every one of the 7 services validates individually when complete" "${all_services_ok}"

env_config::validate_all "${COMPLETE_DIR}" >/dev/null 2>&1
check "env_config::validate_all succeeds when every service's .env is complete" $([ $? -eq 0 ] && echo 1 || echo 0)
...
ERR="$(env_config::validate_service "${MISSING_FILE_DIR}" "hive" 2>&1 >/dev/null)"
RC=$?
...
env_config::validate_all "${MISSING_FILE_DIR}" >/dev/null 2>&1
check "env_config::validate_all fails when any single service's file is missing" $([ $? -ne 0 ] && echo 1 || echo 0)
...
ERR="$(env_config::validate_service "${MISSING_KEY_DIR}" "auth" 2>&1 >/dev/null)"
RC=$?
...
env_config::validate_service "${MISSING_KEY_DIR}" "apiary" >/dev/null 2>&1
...
env_config::validate_service "${EMPTY_KEY_DIR}" "media" >/dev/null 2>&1
...
ERR="$(env_config::validate_service "${BAD_MODE_DIR}" "apiary" 2>&1 >/dev/null)"
RC=$?
...
# --- 8. env_config::missing_required and validate_service never print a
#     secret's value, only key names ---

MISSING_OUTPUT="$(env_config::missing_required "${MISSING_KEY_DIR}/auth.env" "auth")"
echo "${MISSING_OUTPUT}" | grep -qF "="
check "env_config::missing_required output contains no '=' (names only, never values)" $([ $? -ne 0 ] && echo 1 || echo 0)

if env_config::validate_service "${COMPLETE_DIR}" "auth" 2>&1 >/dev/null | grep -qF "jwt-key"; then
  check "env_config::validate_service never echoes a secret's value" 0
else
  check "env_config::validate_service never echoes a secret's value" 1
fi

# --- 9. env_config::is_secret_key recognizes each service's own
#     required keys, and rejects a key that service doesn't own ---

all_recognized=1
for key in ${ENV_REQUIRED_KEYS[auth]}; do
  env_config::is_secret_key "auth" "${key}" || all_recognized=0
done
check "env_config::is_secret_key recognizes all of auth's required keys" "${all_recognized}"

env_config::is_secret_key "auth" "STORAGE_BUCKET"
check "env_config::is_secret_key rejects a key auth doesn't own (media's STORAGE_BUCKET)" $([ $? -ne 0 ] && echo 1 || echo 0)

env_config::is_secret_key "gateway" "PUBLIC_DOMAIN"
check "env_config::is_secret_key rejects a non-secret, non-owned key" $([ $? -ne 0 ] && echo 1 || echo 0)
